                    ...
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">                    <div class="row">
                        ...
                        <div class="form-group col-12 col-md-6">
                            <label for="type">Tipo:</label>
                                ...
                                <option value="Encargado">Encargado tienda</option>
...
<div class="modal fade" id="modal_editar_usuario" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    ...
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link" data-widget="pushmenu" href="/auditoria#" role="button"><i class="fas fa-bars"></i></a>
      </li>
    ...
    <ul class="navbar-nav ml-auto">
        ...
        <a class="nav-link" data-toggle="dropdown" href="/auditoria#">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          ...
          <div id="sugerencia28">
            <a href="/auditoria#" class="dropdown-item">
          ...
          <div id="sugerencia2997">
            ...
            <li class="nav-item">
        <a class="nav-link" href="/auditoria#" data-toggle="modal" data-target="#updatePasswordModal">
      ...
      .catch(error => {
        ...
        alert('Error al actualizar la contraseña. Verifique la consola para más detalles.');
			...
			<aside class="main-sidebar elevation-4 sidebar-light-warning">
				<!-- Brand Logo -->
				<a href="/auditoria#" class="brand-link" style="text-align: center ;">
  ...
  <div class="user-panel mt-3 pb-3 mb-3 d-flex">
    ...
    <div class="info">
      <a href="/auditoria#" class="d-block">Administrador</a>
...
<li class="nav-item">
    ...
    <div class="col-sm-6">
        <h1 class="m-0 text-dark txt-title">{{ $sucursal->nombre }} &mdash; Auditoría no. {{ $evento->no_auditoria }}
            @if ($evento->fecha_fin)
    ...
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item txt-blue active">{{ $sucursal->nombre }}</li>
        </ol>
    </div><!-- /.col -->
</div><!-- /.row -->
					</div><!-- /.container-fluid -->
							...
							<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
								<div class="card">
									<div class="card-header">
										<h5 class="card-title" style="font-weight: bold">Productos</h5>
										<div class="card-tools">
											<form method="post" action="{{ url('/auditoria/reporte-auditoria') }}" style="display:inline-block">
												<input type="hidden" name="_token" value="{{ csrf_token() }}">
												<input type="hidden" name="id" value="{{ $evento->id }}">
												<button class="btn btn-outline-success btn-sm" type="submit" title="Resumen de auditoría"><i class="fa fa-file-excel"></i> REPORTE EXCEL</button>
											</form>
											<form method="post" action="{{ url('/auditoria/reporte-detallado') }}" style="display:inline-block; margin-left:5px;">
												<input type="hidden" name="_token" value="{{ csrf_token() }}">
												<input type="hidden" name="id" value="{{ $evento->id }}">
												<button class="btn btn-success btn-sm" type="submit" title="Detalle de todas las entradas y salidas"><i class="fa fa-file-excel"></i> REPORTE DETALLADO</button>
											</form>
											<form method="post" action="{{ url('/auditoria/reporte/pdf') }}" style="display:inline-block; margin-left:5px;">
												<input type="hidden" name="_token" value="{{ csrf_token() }}">
												<input type="hidden" name="id" value="{{ $evento->id }}">
												<button class="btn btn-default btn-sm" type="submit" title="Reporte PDF"><i class="fa fa-file-pdf"></i> REPORTE PDF</button>
											</form>
											<button type="button" class="btn btn-success btn-sm" id="btnFinalizarAuditoria" @if($evento->fecha_fin) disabled @endif style="margin-left:5px;">
									...
									<div class="card-body">
										<div class="table-responsive">
											<table id="tblProductos" class="table table-bordered table-sm table-hover">
...
<!-- Modal Auditar Producto -->
<div class="modal fade" id="modalAuditoriaProducto" tabindex="-1" role="dialog" aria-labelledby="modalAuditoriaProductoLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="titulo_modal" style="color: green"></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="formAuditoriaProducto" method="POST" action="#">
                <div class="modal-body">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" id="auditoria_auditoria_producto_id" name="auditoria_producto_id">
                    ...
                    <table class="table table-bordered table-sm">
                        ...
                        <tbody id="tbl_auditoria_producto"></tbody>
                    </table>
                    ...
                    <table class="table table-bordered table-sm">
                        ...
                        <tbody id="tbl_auditoria_producto_general"></tbody>
                    </table>
  ...
  Copyright &copy;<script>document.write(new Date().getFullYear());</script> <a href="/auditoria#" target="_blank">FD3-ACCESORIOS</a> | Icons <a href="https://iconos8.es/">Icons8</a>
...
<script>
    window.localId = {{ $evento->local_id }};
    window.auditoriaId = {{ $evento->id }};
    window.auditado = {{ $evento->fecha_fin ? 'true' : 'false' }};
    window.routeGetProductosByLocalID = '/productos/getProductosByLocalId';
    window.routeProductoAuditar = '/auditoria/producto-auditar';
    window.routeProductoAuditado = '/auditoria/producto-auditado';
</script>
<script src="/js/app-auditoria-local.js%3Fid=2f703a70389bc3b1d29475009dda3e8a"></script>
<script>
    document.getElementById('btnFinalizarAuditoria').addEventListener('click', function () {
        ...
        fetch("{{ url('/auditoria/'.$evento->id.'/finalizar') }}", {
            method: 'POST',
