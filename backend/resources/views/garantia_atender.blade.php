...
<head>
    ...
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link" data-widget="pushmenu" href="/garantia-atender#" role="button"><i class="fas fa-bars"></i></a>
      </li>
    ...
    <ul class="navbar-nav ml-auto">
        ...
        <a class="nav-link" data-toggle="dropdown" href="/garantia-atender#">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          ...
          <div id="sugerencia28">
            <a href="/garantia-atender#" class="dropdown-item">
          ...
          <div id="sugerencia2997">
            ...
            <li class="nav-item">
        <a class="nav-link" href="/garantia-atender#" data-toggle="modal" data-target="#updatePasswordModal">
      ...
      .catch(error => {
        ...
        alert('Error al actualizar la contraseña. Verifique la consola para más detalles.');
			...
			<aside class="main-sidebar elevation-4 sidebar-light-warning">
				<!-- Brand Logo -->
				<a href="/garantia-atender#" class="brand-link" style="text-align: center ;">
  ...
  <div class="user-panel mt-3 pb-3 mb-3 d-flex">
    ...
    <div class="info">
      <a href="/garantia-atender#" class="d-block">Administrador</a>
    ...
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title" style="font-weight: bold">Atender Garantías</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
...
<!-- Modal Ver Última Compra -->
<div class="modal fade" id="lastPurchaseModal" tabindex="-1" role="dialog" aria-labelledby="lastPurchaseModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="lastPurchaseModalLabel">Última Compra</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p><strong>Cliente:</strong> <span id="lastPurchaseCliente"></span></p>
                <p><strong>Producto:</strong> <span id="lastPurchaseProducto"></span></p>
                <p><strong>Fecha de Compra:</strong> <span id="lastPurchaseFecha"></span></p>
                <p><strong>Días Transcurridos:</strong> <span id="lastPurchaseDias"></span></p>
                <p><strong>Cantidad:</strong> <span id="lastPurchaseCantidad"></span></p>
                <p><strong>Precio:</strong> <span id="lastPurchasePrecio"></span></p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
            </div>
...
<div class="modal fade" id="statusModal" tabindex="-1" role="dialog" aria-labelledby="statusModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form id="statusForm" action="/garantia-update-status" method="POST">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">                <input type="hidden" name="_method" value="PUT">                <div class="modal-header">
                    <h5 class="modal-title" id="statusModalLabel">Cambiar Estatus</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
            ...
            <form id="actionForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group" id="totalGroup" style="display:none;">
                        <label for="total">Total (default precio actual de acuerdo a nivel)</label>
                        <input type="number" class="form-control" id="total" name="total">
                    </div>
                    <button type="button" class="btn btn-info" id="validateExistence">Validar Existencia</button>
                    <table class="table mt-3" id="existenceTable" style="display:none;">
                        <thead>
                            <tr>
                                <th>Local</th>
                                <th>Existencia</th>
...
<div class="modal fade" id="summaryModal" tabindex="-1" role="dialog" aria-labelledby="summaryModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="summaryModalLabel">Resumen de Garantía</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <p><strong>Producto:</strong> <span id="summaryProducto"></span></p>
                <p><strong>Cliente:</strong> <span id="summaryCliente"></span></p>
                <p><strong>Motivo:</strong> <span id="summaryMotivo"></span></p>
                <p><strong>Solución:</strong> <span id="summarySolucion"></span></p>
...
<div class="modal fade" id="lastPurchaseModal" tabindex="-1" role="dialog" aria-labelledby="lastPurchaseModalLabel" aria-hidden="true">
  ...
  Copyright &copy;<script>document.write(new Date().getFullYear());</script> <a href="/garantia-atender#" target="_blank">FD3-ACCESORIOS</a> | Icons <a href="https://iconos8.es/">Icons8</a>
  <div class="float-right d-none d-sm-inline-block">
    ...
    $('#statusModal').on('show.bs.modal', function(event) {
        ...
        var garantiaId = button.data('id');
        var status = button.data('status');
        var modal = $(this);
        modal.find('#garantia_id').val(garantiaId);
        modal.find('#status').val(status);
    });

    $('#lastPurchaseModal').on('show.bs.modal', function(event) {
        var button = $(event.relatedTarget);
        var clienteId = button.data('cliente');
        var productoId = button.data('producto');
        var modal = $(this);
        modal.find('#lastPurchaseCliente').text('Cargando...');
        modal.find('#lastPurchaseProducto').text('');
        modal.find('#lastPurchaseFecha').text('');
        modal.find('#lastPurchaseDias').text('');
        modal.find('#lastPurchaseCantidad').text('');
        modal.find('#lastPurchasePrecio').text('');
        $.ajax({
            ...
            success: function(response) {
                var fechaCompra = new Date(response.fecha);
                var diasTranscurridos = Math.floor((new Date() - fechaCompra) / (1000 * 60 * 60 * 24));
                modal.find('#lastPurchaseProducto').text(response.producto);
                modal.find('#lastPurchaseCliente').text(response.cliente);
                modal.find('#lastPurchaseFecha').text(response.fecha);
                modal.find('#lastPurchaseDias').text(diasTranscurridos);
                modal.find('#lastPurchaseCantidad').text(response.cantidad);
                modal.find('#lastPurchasePrecio').text(response.precio);
            },
            error: function(xhr) {
                modal.find('#lastPurchaseCliente').text((xhr.responseJSON && xhr.responseJSON.error) || 'No se encontró una compra previa.');
            }
        });
    });
    ...
    $('#summaryModal').on('show.bs.modal', function(event) {
        ...
        var modal = $(this);
        modal.find('#summaryProducto').text(producto);
        modal.find('#summaryCliente').text(cliente);
        modal.find('#summaryMotivo').text(motivo);
        modal.find('#summarySolucion').text(solucion);
        modal.find('#summaryTotal').text(total);
    });
    ...
    $('#lastPurchaseModal').on('show.bs.modal', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                var diasTranscurridos = Math.floor((fechaRegistro - fechaCompra) / (1000 * 60 * 60 * 24));
    ...
    $('#action').on('change', function() {
        ...
        var producto = $('#validateExistence').data('producto');
    ...
    $('#validateExistence').on('click', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                var tableBody = $('#existenceTableBody');
                tableBody.empty();
                response.forEach(function(item) {
                    tableBody.append('<tr><td>' + item.local + '</td><td>' + item.existencia + '</td></tr>');
                });
                $('#existenceTable').show();
            }
        });
