...
<head>
    ...
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link" data-widget="pushmenu" href="/configuracion#" role="button"><i class="fas fa-bars"></i></a>
      </li>
    ...
    <ul class="navbar-nav ml-auto">
        ...
        <a class="nav-link" data-toggle="dropdown" href="/configuracion#">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          ...
          <div id="sugerencia2997">
            ...
            <li class="nav-item">
        <a class="nav-link" href="/configuracion#" data-toggle="modal" data-target="#updatePasswordModal">
      ...
      .catch(error => {
        ...
        alert('Error al actualizar la contraseña. Verifique la consola para más detalles.');
			...
			<aside class="main-sidebar elevation-4 sidebar-light-warning">
				<!-- Brand Logo -->
				<a href="/configuracion#" class="brand-link" style="text-align: center ;">
  ...
  <div class="user-panel mt-3 pb-3 mb-3 d-flex">
    ...
    <div class="info">
      <a href="/configuracion#" class="d-block">Administrador</a>
            ...
            <a href="/depositos-consulta" id="menu_depositos_consulta" class="nav-link">
              ...
              <p>
                Consulta depósitos
                                        ...
                                        <li class="nav-item">
            <a href="/whatsapp" id="menu_whatsapp" class="nav-link active">
              <i style="padding-right:15px;">
...
<li class="nav-item">
	...
	<div class="col-sm-6">
		<ol class="breadcrumb float-sm-right">
			...
			<li class="breadcrumb-item txt-blue active">WhatsApp</li>
		</ol>
	</div>
</div>
					</div><!-- /.container-fluid -->
...
<div class="row">
    <div class="col-md-4">
        <div class="card">
            <div class="card-header"><strong>Conexión del bot</strong></div>
            <div class="card-body text-center" id="bot-status-box">
                <p>Cargando estado...</p>
            </div>
            <div class="card-footer">
                <button class="btn btn-sm btn-secondary" onclick="cargarEstado()">Actualizar</button>
                <button class="btn btn-sm btn-secondary" onclick="sincronizar()">Sincronizar socios</button>
            </div>
        </div>

        <div class="card">
            <div class="card-header"><strong>Enviar catálogo</strong></div>
            <div class="card-body">
                <div class="form-group">
                    <label>ID de cliente (real, de Cartera de Clientes)</label>
                    <input type="number" class="form-control" id="cliente_id_catalogo">
                </div>
                <button class="btn btn-primary" onclick="enviarCatalogo()">Enviar</button>
                <div id="enviar-catalogo-result" class="mt-2"></div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header"><strong>Pedidos desde el catálogo de WhatsApp</strong></div>
            <div class="card-body">
                <table class="table table-sm table-hover">
                    ...
                    @forelse($pedidos as $p)
                        <tr id="pedido-row-{{ $p->id }}">
                            <td>WA-{{ $p->id }}</td>
                            <td>{{ $p->cliente->nombre ?? ('Cliente #'.$p->cliente_id) }}</td>
                            <td>${{ number_format($p->total, 2) }}</td>
                            <td><span class="badge badge-info" id="pedido-estado-{{ $p->id }}">{{ $p->estado }}</span></td>
                            <td>{{ $p->created_at }}</td>
                            <td>
                                @if($p->estado === 'pendiente')
                                    <button class="btn btn-sm btn-success" onclick="confirmarPedido({{ $p->id }})">Confirmar</button>
                                @elseif(in_array($p->estado, ['confirmado','enviado']))
                                    <button class="btn btn-sm btn-primary" onclick="avanzarPedido({{ $p->id }})">Avanzar</button>
                                @endif
                                @if(!in_array($p->estado, ['completado','cancelado']))
                                    <button class="btn btn-sm btn-danger" onclick="cancelarPedido({{ $p->id }})">Cancelar</button>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="6">Sin pedidos todavía.</td></tr>
                    @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header"><strong>Prospectos</strong></div>
                    <div class="card-body" id="prospectos-box"><p>Cargando...</p></div>
                </div>
            </div>
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header"><strong>Quejas / sugerencias</strong></div>
                    <div class="card-body" id="quejas-box"><p>Cargando...</p></div>
                </div>
            </div>
        </div>
    </div>
...
<script>
function waApi(url, opts) {
    opts = opts || {};
    var headers = { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'), 'Content-Type': 'application/json' };
    return fetch(url, Object.assign({}, opts, { headers: Object.assign(headers, opts.headers || {}) })).then(function(r) { return r.json(); });
}

function cargarEstado() {
    waApi('/whatsapp/status').then(function(d) {
        var html = '';
        if (d.status === 'conectado') {
            html = '<p class="text-success"><i class="fas fa-check-circle"></i> Conectado</p>';
        } else if (d.qr) {
            html = '<p>Escanea este código con WhatsApp (Dispositivos vinculados):</p><img src="' + d.qr + '" style="max-width:100%">';
        } else {
            html = '<p class="text-muted">Estado: ' + (d.status || 'desconocido') + '</p>';
        }
        document.getElementById('bot-status-box').innerHTML = html;
    }).catch(function() {
        document.getElementById('bot-status-box').innerHTML = '<p class="text-danger">No se pudo consultar el bot.</p>';
    });
}

function sincronizar() {
    waApi('/whatsapp/sync', { method: 'POST' }).then(function(d) {
        alert(d.ok ? ('Sincronizado: ' + d.created + ' nuevos, ' + d.updated + ' actualizados de ' + d.total) : (d.error || 'Error'));
    });
}

function enviarCatalogo() {
    var cliente_id = document.getElementById('cliente_id_catalogo').value;
    waApi('/whatsapp/enviar-catalogo', { method: 'POST', body: JSON.stringify({ cliente_id: cliente_id }) }).then(function(d) {
        document.getElementById('enviar-catalogo-result').innerHTML = d.ok
...
function confirmarPedido(id) {
    if (!confirm('¿Confirmar este pedido y crear la venta real?')) return;
    waApi('/whatsapp/pedidos/' + id + '/confirmar', { method: 'POST' }).then(function(d) {
        if (d.ok) { location.reload(); } else { alert(d.message || 'Error'); }
    });
}
function avanzarPedido(id) {
    waApi('/whatsapp/pedidos/' + id + '/avanzar', { method: 'POST', body: JSON.stringify({ accion: 'avanzar' }) }).then(function(d) {
        if (d.ok) { location.reload(); } else { alert(d.message || 'Error'); }
    });
}
function cancelarPedido(id) {
    if (!confirm('¿Cancelar este pedido?')) return;
    waApi('/whatsapp/pedidos/' + id + '/avanzar', { method: 'POST', body: JSON.stringify({ accion: 'cancelar' }) }).then(function(d) {
        if (d.ok) { location.reload(); } else { alert(d.message || 'Error'); }
    });
}

function cargarProspectos() {
    waApi('/whatsapp/prospectos').then(function(d) {
        var rows = (d.prospectos || []).map(function(p) { return '<div>' + (p.nombre || '(sin nombre)') + ' — ' + p.telefono_whatsapp + ' — ' + (p.motivo || '') + '</div>'; }).join('');
        document.getElementById('prospectos-box').innerHTML = rows || '<p class="text-muted">Sin prospectos.</p>';
    }).catch(function() { document.getElementById('prospectos-box').innerHTML = '<p class="text-danger">No disponible.</p>'; });
}
function cargarQuejas() {
    waApi('/whatsapp/quejas').then(function(d) {
        var rows = (d.quejas || []).map(function(q) { return '<div>' + (q.socio_nombre || '(?)') + ': ' + q.texto + '</div>'; }).join('');
        document.getElementById('quejas-box').innerHTML = rows || '<p class="text-muted">Sin quejas.</p>';
    }).catch(function() { document.getElementById('quejas-box').innerHTML = '<p class="text-danger">No disponible.</p>'; });
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  Copyright &copy;<script>document.write(new Date().getFullYear());</script> <a href="/configuracion#" target="_blank">FD3-ACCESORIOS</a> | Icons <a href="https://iconos8.es/">Icons8</a>
  <div class="float-right d-none d-sm-inline-block">
	...
	<script src="/plugins/jquery-mapael/maps/usa_states.min.js"></script-->
		...
		<script>
        window.updatelocaldata = '/actualiza_datos_local';
</script>
<script src="js/app-configuration.js%3Fid=db2a3af0ce7629eb13db333ba53e2c4c"></script>
		<script>
