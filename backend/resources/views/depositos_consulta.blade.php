...
<head>
    ...
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link" data-widget="pushmenu" href="/depositos-consulta#" role="button"><i class="fas fa-bars"></i></a>
      </li>
    ...
    <ul class="navbar-nav ml-auto">
        ...
        <a class="nav-link" data-toggle="dropdown" href="/depositos-consulta#">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          ...
          <div id="sugerencia28">
            <a href="/depositos-consulta#" class="dropdown-item">
          ...
          <div id="sugerencia2997">
            ...
            <li class="nav-item">
        <a class="nav-link" href="/depositos-consulta#" data-toggle="modal" data-target="#updatePasswordModal">
      ...
      .catch(error => {
        ...
        alert('Error al actualizar la contraseña. Verifique la consola para más detalles.');
			...
			<aside class="main-sidebar elevation-4 sidebar-light-warning">
				<!-- Brand Logo -->
				<a href="/depositos-consulta#" class="brand-link" style="text-align: center ;">
  ...
  <div class="user-panel mt-3 pb-3 mb-3 d-flex">
    ...
    <div class="info">
      <a href="/depositos-consulta#" class="d-block">Administrador</a>
...
<li class="nav-item">
    ...
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item txt-blue active">Consulta de Depósitos</li>
        </ol>
    </div><!-- /.col -->
</div><!-- /.row -->
					</div><!-- /.container-fluid -->
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Consulta de Depósitos</h3>
                            <div class="card-tools">
                                <input type="date" id="fechaConsulta" class="form-control" value="2026-08-12">
                            </div>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
    ...
    <div class="modal fade" id="ventasModal" tabindex="-1" role="dialog" aria-labelledby="ventasModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ventasModalLabel">Ventas del Asesor</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
  ...
  Copyright &copy;<script>document.write(new Date().getFullYear());</script> <a href="/depositos-consulta#" target="_blank">FD3-ACCESORIOS</a> | Icons <a href="https://iconos8.es/">Icons8</a>
  <div class="float-right d-none d-sm-inline-block">
    ...
    $(document).ready(function() {
        $('#menu_depositos_consulta').attr('class', 'nav-link active');
        ...
        $('#fechaConsulta').on('change', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    var depositosTable = $('#depositosTable');
                    depositosTable.empty();
                    response.depositos.forEach(function(deposito) {
                        depositosTable.append(
                            '<tr>' +
                                '<td>' + deposito.user.name + '</td>' +
                                '<td>' + deposito.monto + '</td>' +
                                '<td class="fecha-deposito" style="cursor:pointer" data-fecha="' + deposito.fecha + '" data-asesor="' + deposito.user.id + '"><span class="badge badge-warning">' + deposito.fecha + '</span></td>' +
                                '<td><a href="/storage/' + deposito.comprobante + '" target="_blank">Ver Comprobante</a></td>' +
                            '</tr>'
                        );
                    });
                }
        ...
        $(document).on('click', '.fecha-deposito', function() {
            var fecha = $(this).data('fecha');
            var asesorId = $(this).data('asesor');
            ver_ventas(asesorId, fecha);
        });

        function ver_ventas(asesorId, fecha) {
            $('#ventasTable').empty();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    response.ventas.forEach(function(venta) {
                        ...
                        var tipoVentaClass = venta.tipo_venta == 'CREDITO' ? 'text-orange' : 'text-green';
                        ventasTable.append(
                            '<tr>' +
                                '<td>' + venta.no_venta + '</td>' +
                                '<td class="' + tipoVentaClass + '">' + venta.tipo_venta + '</td>' +
                                '<td>' + venta.cliente + '</td>' +
                    ...
                    $('#ventasModalLabel').html('Ventas del Asesor <b style="color:green">' + response.vendedor + '</b> del día ' + fecha);
